    ...
    if G is not None:
            ...
            if autor_sel:
                ...
                with col1:
                    ...
                    with col_b:
                        ...
                        st.metric("👑 Como Principal", principal)
                        st.metric("🔗 Como Secundario", secundario)
                    
                    # Campo principal y citaciones
                    if campo_princ:
                        st.markdown(f"**🎯 Campo Principal:** {campo_princ}")
                    
                    # Calcular citaciones recibidas
                    ...
                    st.markdown(f"**📈 Citaciones Recibidas:** {citaciones_recibidas}")
        ...
        with st.expander("🌐 **Comunidades**", expanded=False):
            ...
            if G is not None:
                ...
                with col4:
                    st.metric("Pequeñas (≤3)", len(pequenas))
                ...
                comm_sel = st.selectbox("Selecciona una comunidad para explorar", comm_options, key=f"selector_comunidad_autores_{hash(str(st.session_state.get('red_seleccionada', 'default'))) % 1000}")
                if comm_sel:
                    ...
                    with col_left:
                        ...
                        # Sección de autores de la comunidad
                        mostrar_autores = st.toggle("👥 Mostrar autores de la comunidad", key=f"toggle_autores_{idx}")
                        if mostrar_autores:
                            st.markdown("**Autores:**")
                        ...
                        if tipo_red == "Red Autor-Institución":
                            ...
                            if instituciones_comunidad:
                                mostrar_instituciones = st.toggle(f"🏛️ Mostrar instituciones de la comunidad ({len(instituciones_comunidad)})", key=f"toggle_instituciones_{idx}")
                            ...
                            if campos_comunidad:
                                mostrar_campos = st.toggle(f"🎓 Mostrar campos de estudio de la comunidad ({len(campos_comunidad)})", key=f"toggle_campos_{idx}")
                                if mostrar_campos:
                        ...
                        if papers_comunidad:
                            mostrar_articulos = st.toggle(f"📄 Mostrar artículos de la comunidad ({len(papers_comunidad)})", key=f"toggle_articulos_{idx}")
                    ...
                    with col_right:
                        ...
                        # --- Resumen conciso de la comunidad ---
                        ...
                        # Resumen en formato de puntos clave
                        st.markdown("**📊 Resumen de la Comunidad**")
                        
                        col_a, col_b = st.columns(2)
                        with col_a:
                            st.markdown(f"• **Tamaño:** {len(comm_nodes)} autores (#{pos_tam} de {len(communities)})")
                            st.markdown(f"• **Productividad:** {total_papers} artículos (#{pos_papers} de {len(communities)})")
